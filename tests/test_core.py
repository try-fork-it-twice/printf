...
from itmo_ics_printf.core.datatype import TaskCreate, TaskSwitched, TraceLog
...
def test_tracelog_load_basic_valid_file(tmp_path: Path) -> None:
    ...
    gen().task_create(
        timestamp=0, task_number=1, task_name="Alpha"
    ).task_switched_in(timestamp=5, task_number=1).task_switched_out(
        timestamp=10, task_number=1
    ).task_create(timestamp=15, task_number=2, task_name="Beta").save(path)
...
def test_tracelog_load_missing_event_fields(tmp_path: Path) -> None:
    ...
    gen().task_custom(
        timestamp=1, task_number=0, event_type=0
    ).save(path)
