...
from itmo_ics_printf.core.datatype import TraceLog, TaskCreate, TaskSwitched, TASK_SWITCHED_IN
...
class MatplotlibAdapter():
    def convert(self, tracelog: TraceLog) -> Figure:
        ...
        ax: Axes
        fig, ax = plt.subplots()
        timestamps, task_numbers, event_types, creation_names = self._extract_tracelog_data(tracelog)
        ...
        for task in tasks:
            task_events = [(ts, et) for ts, tn, et in zip(timestamps, task_numbers, event_types) if tn == task]
            ...
            task_name = creation_names[task-1] if (task-1) < len(creation_names) else f'Task {task}'
            
            # Create separate scatter plots for each event type with labels
            create_ts = [ts for ts, et in task_events if et == 'TaskCreate']
            if create_ts:
                sc = ax.scatter(create_ts, [task]*len(create_ts), color=color, marker='o', 
                               s=256, linewidth=2, zorder=3)
            ...
            if in_ts:
                sc = ax.scatter(in_ts, [task]*len(in_ts), color=color, marker='|', 
                               s=256, linewidth=1, zorder=3)
                labels_dict[sc] = [f'TASK_SWITCHED_IN (Task {task})'] * len(in_ts)
                all_scatter.append(sc)
            
            out_ts = [ts for ts, et in task_events if et == 'TASK_SWITCHED_OUT']
            if out_ts:
                sc = ax.scatter(out_ts, [task]*len(out_ts), color=color, marker='|', 
                               s=256, linewidth=1, zorder=3)
    ...
    def _extract_tracelog_data(
        ...
        for event in tracelog.events:
            if isinstance(event, TaskCreate):
                ...
                timestamps.append(event.timestamp)
                task_numbers.append(event.task_number)
                event_type = 'TASK_SWITCHED_IN' if event.event_type == TASK_SWITCHED_IN else 'TASK_SWITCHED_OUT'
